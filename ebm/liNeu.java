...
public class liNeu{
	...
	public liNeu(String Path,String target,String Bcode){	
		mDirs=new String[0];			
		mFiles=new String[0];			
		path=new com.units.CF(Path).path;
		start();		
		...
		int code=intmodi(Bcode);
		new com.units.save().file("gastro/tot.dat","0",false);
		new com.units.save().file("gastro/"+target,"",false);
		new ebm.org("gastro/k_r_ft"+target,code);

	}
	public liNeu(String Path,String target,String Bcode, String def){	
		mDirs=new String[0];			
		mFiles=new String[0];			
		path=new com.units.CF(Path).path;
		start();		
		...
		new com.units.save().file("gastro/tot.dat","0",false);
		new com.units.save().file("gastro/"+target,"",false);
		new ebm.org("gastro/k_r_ft"+target,code,intmodi(def));
	...
	String[]f(String[]v){		
		...
		for(int i= 0;i<v.length;i++){
			...
			time=time.replace('.','_');
			if(f.isFile()){				
				m.add(time+","+f.getName());//+", "+time+" , "+f.length());
			}
		}
	...
	public static void main(String[]args){
		if(args.length>0){
			...
			ein=ein.replace('+','?');			//liNeu gastro/drucker 0309.dat 8241 2
			if(args.length==2){liNeu l=new liNeu(ein,args[1],args[2]);}else
			if(args.length>2){liNeu l=new liNeu(ein,args[1],args[2],args[3]);}
			//l.start();
		} //else new li(".");
	}
}
