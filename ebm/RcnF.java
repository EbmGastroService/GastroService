...
public  class RcnF extends JApplet{//implements Printable{
     String R_Code;
     String Datum;
     static String Kcode;
     String Wcode;
   ...
   String[]slid=new ver6.Rb().slid();  
	...
	public String tfzeileinhalt ="<html><body><center><h1>&copy;Ebm Gastro Service 2013</h1>";
	String[] firma;
	public RcnF (){
	//super("EBM Gastroservice Packet 1.07 (2003-2007)");	
    R_Code = null;
    Datum  = null;
    Kcode  = null;
    Wcode  = null;
    ...
    total=0.0;
    s=new save();	
	//Sto=new JButton("Storno");	
	tfzeileinhalt ="<html><body><center><h1>&copy;Ebm Gastro Service 2013</h1>";
	tf  = new javax.swing.JEditorPane("text/html",tfzeileinhalt);	
    tlz=new JLabel(oprc[7]+new MyZahl().deci(total));
    east = new JPanel ();
    west = new JPanel ();
    tc = new JPanel ();
    tccpanel = new JPanel ();
    ...
    bg1=null;  	
    rwert=new JLabel("<html><font color=red size=+1></font>",JLabel.CENTER);
	ver6.opString	os=new opString();
	os.rc_d();	os.rc_e();os.kd_d();os.kd_e();
	tf.setEditable(false);	
	setTf();		
	firma =new firma().data();   	
	flabel= new ver6.img().label(firma,"logo.gif");
	//init();
	//setVisible(true); //
	//pack();
  }
  public String getAppletInfo() {
    return "EBM Gastroservice Packet 1.011 (2003-2011), by Mourad El bakry";
    }    
    void setTf(){
    	String[]str=new ver6.Rb().rcntf();   
    	String tfzeile=tfzeileinhalt;
    ...
    int solldrucken(int o){
    	int i=0;
    	String str=new sucheDate("gastro/resource/sdrucken.dat").md();
    	if(str.length()==0){str="0";new save().file("gastro/resource/sdrucken.dat",str,false);}
    	if(str=="1"){str=""+new com.options.MyOp().Frage("Rechnung drucken!");}
    	try{i=Integer.parseInt(str);
    	}catch(Exception nfe){i=0;}
    	return i;
    }
    ...
    public String[][] die_ware(){
      ...
      int d = 0;      
      String ws=w.suchen();
      if (ws.length()==0 || ws==null) ws=w.suchen();
      if (ws!="" && ws!=null) {
        String ein = mo.eingabeMenge("<html><font color=blue size=+1> "+w.getw_Code()+"   "+w.getBezeichnung().trim()
                                     +"   "+w.getPreis()+ "<br> Menge: ");
        if (ein.length()==0) {
    ...
    rwert.setText("<html><font color=red size=+1>"+
                       "Total<br>"+new MyZahl().deci(getSaldo())+"</font>");
    ...
    public String rec_sume(){
    	String rzeile=""+R_Code+" Total: "+new MyZahl().deci(getSaldo());    
      return rzeile;
    }
    ...
    public String[][] der_Rechnung(){          
    	rwert.setOpaque(true);rwert.setBackground(Color.white);rwert.validate();
    	rwert.setText("<html><font color=blue size=+1>Total<br>"+new MyZahl().deci(0)+"</font>");
      return der_korb();      
    }
    ...
    public boolean saveControl(String str){
      if( new MyOp().Frage(str)==0){schloss();
      		rwert.setText("");rwert.setOpaque(true);rwert.setBackground(hg);rwert.validate();
      	return true;
      } else {
        int i=new MyOp().Frage("<html><bgcolor=red><font color=white size=+1>Erfasssung Abbrechen! </font>"+
          ...
          if(i==1){schloss();
          	...
          	String tfzeile="<html><body><center><h1>Abbruch!!</h1><font color=red><b> Erfassung ist Fehlgeschlagen!..canceled!!</b></font>";tf.setText(tfzeile);          
          	info.append(" *F");
          	rwert.setText("");rwert.setOpaque(true);rwert.setBackground(hg);rwert.validate();
          	return false;
          }
    ...
    public String R_code(){
      ...
      if(k.Frage(oprc[3]+//"<html><b><font color=green size=+1>Neue Rechnung erstellen</font>"+
    ...
    public void speichern() {     
     ...
     //new save().file("gastro/date/KZ.dat",zeile,true);
    	String tfzeile=tf.getText()+"<br>"+oprc[6];
      tf.setText(tfzeile);//" Daten sind gespeichert");      
    }
    public String[][] lesen() {
      String zeile;
    String[] sicherheit={"RN080506/0113,080506,1002X00011080506,0.0"};
      ...
      setTotal(gtot);
      tlz.setText(oprc[7]+new MyZahl().deci(getTotal()));              
      return(zz);
    }
    ...
    public String[][] storno(){
     ...
     if(erg == -1)return korbDyn();
      ...
      int an = new MyOp().frageO("<html><b>"+oprc[18]+"<br>"+oprc[19]+"<br>"+
                                 oprc[20]+"<br>"+oprc[21]+"<br>"+oprc[22]
    ...
    public String[][] erfassen(){
      String[][]rec = null;     
    String tfzeile="<html><body><center><h1>Neue Rechnung</h1>";//tf.getText();//tfzeileinhalt;	
      tf.setText(tfzeile);
    	tk.setText(""); tr.setText("");tlz.setText("");    	
      String[]derkunde=der_kunde();
      if(derkunde!= null){
        String[]der_k= kfdaten(derkunde);     
      	info.append("\n"+Kcode.substring(0,Kcode.indexOf("X")));
      	tfzeile+="<b>"+oprc[24]+"<font size=+1 color=blue>"+Kcode.substring(0,Kcode.indexOf("X"))+"  </font> hat <font size=+2 color=red> "+kAb+oprc[25]+"</b></font>";
        tf.setText(tfzeile);      //" Rechnung an Kunden Nummer: ""  erfassen"
          ...
          if (derk!="" && derk!=null){                        
            ...
            zumtabele(rec,2);
          	...
          	if(saveControl("Eingabe Korrekt?\n es wird gespeichert!")){
            	new BW(2,"");
            	savek_r(rec);
          		if(solldrucken(0)==0){//Fangfrage soll rechnung gedrucket oder nicht
          			if(myform==1)
          				new GF(""+Kcode,firma,der_k,rec,kopf,Skale);
          			else new com.printer.DF(""+Kcode,firma,der_k,rec,kopf,Skale);
          		}
          		tfzeile+="<br><font size=+1 color=green><b>"+oprc[28]+rec_sume()+oprc[31]+"</b></font>";
                tf.setText(tfzeile);//"\n ist abgeschlossen");
            	tcliste();
            	lesen();//setTotal(total+getSaldo());
            	tlz.setText(rec_sume());//+", OP: "+new MyZahl().deci(getTotal()));//changed am 11/5/2011
            }
          } else{	tfzeile+="<br><font color=red><b> ist abgebruchen ..canceled!!</b></font>";tf.setText(tfzeile);}//ist abgebrochen!!
        }else {
        	tfzeile+="<br><font color=red><b> ist abgebruchen!! ..Canceled!!</b></font>";//ist abgebrochen!!
        	tf.setText(tfzeile);
        }
      }
      return rec;
    ...
    public String[][] gelesen(){
      ...
      String[][] v = lesen();
      zumtabele(v,1);
        tlz.setText(oprc[34]+v.length+oprc[35]+new MyZahl().deci(getTotal()));
      return v;
    }
    public int wo(){
      MyOp mo = new MyOp();
    ...
    class Mykd implements ActionListener {
      public void actionPerformed ( ActionEvent e ) {
        ...
        if (fr.equals("Offene Posten")){
        	gelesen();tlz.setText("Offene Posten: "+(float)total+" :> ");
        }
        ...
        if (fr.equals("Tageserlos")){
          ...
          tf.setText(tfzeileinhalt+"<br>"+oprc[36]+te.length+oprc[37]+kb.rec_sume(te.length)+"<br>"+oprc[40]+"<br>"+oprc[41]);
                     //""+" Der Tageserlose hat "+te.length+" erfasste Buchungen gefunden."+kb.rec_sume(te.length)+
        ...
        if (fr.equals("F-Bericht")){tf.setText(tfzeileinhalt+"<br><font Color=blue>"+new FL().Bericht()+"</font>");}
  ...
  //ntitle.setTitleJustification(TitledBorder.CENTER);    
   ...
   // tf.setEditable(false);
    ...
    if(hg!=null)west.setBackground(hg);
   	if(hg!=null)rwert.setOpaque(true);rwert.setBackground(hg);rwert.validate();
   ...
   void paneleninit(){
    ...
    jb = new JPanel (new GridLayout((ok.length),0));   
   ...
   class Keys implements KeyListener {
    ...
    //  tf.setText(" "+keyString);
      if (e.isActionKey()) {      
        ...
        if(keyString.equals("122" )&& modifiers==128){          //Strg_F11
          new ebm.Defrag().entMon();
        }
    ...
    void delZeile(){
    	 int interval=A1();interval++;    	
          //if(interval<3){
          	System.out.println(interval+" Storno Versuch!");
 ...
 public void init(){           
  ...
  //  new kur();    
    ...
    //flabel=new ver6.Rb(0).label(firma); 
 	/*new JLabel("<html><center><font size=+1 font color=blue>"+// face='ZurichCalligraphic'> "+
                       firma[0]+"</font>"+
    ...
    west.add(jpanel(tcliste()),BorderLayout.PAGE_START);//new JScrollPane (tc)); 
 	rwert.setPreferredSize(new Dimension(120,100));
 	rwert.setOpaque(true);rwert.setBackground(hg);rwert.validate();
    west.add(rwert);//,BorderLayout.PAGE_END);  
 	//west.add(new JLabel(""));//,BorderLayout.PAGE_END);  
 	west.add(new ver6.img().label(null,"logo.gif",150,100),BorderLayout.PAGE_END);//panenew firma().data()
  ...
  lastrow.add(c,BorderLayout.EAST);   
    ...
    getContentPane().add(lastrow,BorderLayout.PAGE_END);
  ...
  public static void main(String[] args) {new RcnF();}
}
